...
const oddAverage = oddTotal / Object.values(game.odds).length;
...
for (const min of minutesArray) {
  intervalTotal += min - prev;
  prev = min;
}
console.log(`Total minutes: ${intervalTotal}`);
console.log(
  `An event happened, on average, every ${
    intervalTotal / minutesArray.length
  } minutes`
);
...
document.querySelector('button').addEventListener('click', function () {
  ...
  //     else {
  //       newArray[i][j] =
  //         newArray[i][j][0].toUpperCase() + newArray[i][j].slice(1);
  //     }
  //   }
  //   console.log(
  //     newArray[i].join('').padEnd(20, ' ') + '✅'.repeat(i + 1).padEnd(5)
  //   );
  // }
});
